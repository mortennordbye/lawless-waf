...
seed: .env ## Generate synthetic sample days for an offline trial (Front Door + Application Gateway; the 25th has the FP fixed, so the diff has something to show)
	$(DC) run --rm api sh -c 'python -m lawless_waf.sample "$$DATA_DIR/frontdoor/2026-06-24/merged.json" && \
