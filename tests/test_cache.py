...
def test_delete_removes_the_count_sidecar(tmp_path):
    ...
    assert cache.delete(f"{WT}:{DATE}") is True
    assert not ds.count_path.exists()
...
def test_acquire_lock_blocks_while_owner_is_live(tmp_path):
    ...
    cache = DatasetCache(tmp_path)
    fd = cache.acquire_lock(WT, DATE, HOUR)
    try:
        with pytest.raises(FileExistsError):
            cache.acquire_lock(WT, DATE, HOUR)
    finally:
        os.close(fd)
        cache.lock_path(WT, DATE, HOUR).unlink()
...
def test_acquire_lock_reclaims_dead_owner(tmp_path):
    ...
    cache = DatasetCache(tmp_path)
    lock = cache.lock_path(WT, DATE, HOUR)
    lock.parent.mkdir(parents=True, exist_ok=True)
    lock.write_text("2147483647")  # a PID that isn't running

    fd = cache.acquire_lock(WT, DATE, HOUR)  # reclaims instead of raising
    os.close(fd)
...
def test_acquire_lock_reclaims_aged_out(tmp_path):
    ...
    fd = cache.acquire_lock(WT, DATE, HOUR, stale_after=1.0)  # reclaims on age
    os.close(fd)
    lock.unlink()
...
def test_acquire_lock_never_steals_our_own_live_lock(tmp_path):
    ...
    try:
        with pytest.raises(FileExistsError):
            cache.acquire_lock(WT, DATE, HOUR, stale_after=1.0)
    finally:
        os.close(fd)
        lock.unlink()
